    ...
    <Combobox value={selected} onChange={onSelect}>
      {({ open }) => (
        <div className={classNames('relative w-full', bottom && open && 'mb-60')}>
          ...
          >
            <Combobox.Input
              className="w-full text-sm leading-5 focus:ring-0 focus:outline-none focus:border-0 border-0 p-0"
              onBlur={onBlur}
              displayValue={(d?: Channel) => d?.name ?? ''}
              onChange={(event) => setQuery(event.target.value)}
              placeholder={`우측 버튼을 클릭하여 채널을 선택하세요`}
            />
                    ...
                    {({ selected, active }) => (
                      <>
                        ...
                        >
                          ...
                          {channel.parentId && (
                            <span className="ml-2 text-ellipsis text-description-color">
